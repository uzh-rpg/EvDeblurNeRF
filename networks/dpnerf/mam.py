...
class MotionAggregationModule(nn.Module):
    def __init__(self, in_channels, k, num_motion):
        super(MotionAggregationModule, self).__init__()
        self.Corr = CorrelationModule(in_channels)
    ...
    def forward(self, x_global, x_local):
        ...
        x_local = rearrange(x_local, '(N_ray N_motion) N_sample N_dim -> N_ray N_motion N_sample N_dim',
                            N_motion=(self.num_motion + 1))
